-- ============================================================
--  Hyejin Lee Portfolio — Supabase schema
...
create table if not exists public.projects (
  ...
  year        text,
  body        text,
  -- 이미지가 없을 때 코드로 그리는 그래픽 설정
  motif       text not null default 'grid',       -- arcs|stack|grid|type|waves|beads|checker|blob|stripes
  pal         text[] not null default array['#EFEFEC','#16181C','#4A6CF0'],
  word        text,
  -- 실제 이미지를 쓸 때 (Storage 공개 URL). 있으면 그래픽 대신 이미지가 나옵니다.
  cover_url   text,
  images      text[] not null default '{}',
  credits     jsonb  not null default '[]'::jsonb, -- [["Client","OLLIM"], ...]
  sort_order  int    not null default 0,
  published   boolean not null default true,
  created_at  timestamptz not null default now()
);

create index if not exists projects_order_idx on public.projects (sort_order, id);

-- ---------- 2. 사이트 설정 (About / Process / Contact) ----------
...
-- 쓰기 정책은 일부러 만들지 않았습니다.
-- 공개키로는 수정이 불가능하고, 대시보드나 service_role 키로만 편집됩니다.

-- ---------- 4. 이미지 보관용 Storage 버킷 ----------
